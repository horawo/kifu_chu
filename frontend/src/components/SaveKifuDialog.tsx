...
const SaveKifuDialog: React.FC<SaveKifuDialogProps> = ({ 
    moves, kifuId, initialKifuName = '', initialIsPublic = true, 
    initialSenteName = '先手', initialGoteName = '後手', onClose 
}) => {
    ...
    const handleSave = async (asNew: boolean = false) => {
        ...
        try {
            ...
            const result = await api.saveKifu(kifuName.trim(), csaText, isPublic, targetId, finalSente, finalGote);

            if (result.success) {
                alert(targetId ? '棋譜を上書き保存しました！' : '棋譜を新規保存しました！');
                onClose(finalSente, finalGote);
                // Optionally redirect or refresh? 
                // For now, just close and let the user see the current board.
            } else {
                ...
                <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end', flexWrap: 'wrap' }}>
                    ...
                    <button
                        ...
                        style={{
                            ...
                            border: '1px solid #ccc',
                            borderRadius: '4px',
                            backgroundColor: moves.length === 0 ? '#eee' : '#fff',
                    ...
                    <button
                        ...
                        disabled={moves.length === 0 || saving}
                        style={{
                            padding: '8px 24px',
                            fontSize: '14px',
                            border: 'none',
                            borderRadius: '4px',
                            backgroundColor: moves.length === 0 || saving ? '#ccc' : (kifuId ? '#007bff' : '#28a745'),
                            color: '#fff',
                            cursor: moves.length === 0 || saving ? 'not-allowed' : 'pointer',
                            fontWeight: '600'
                        }}
                    >
